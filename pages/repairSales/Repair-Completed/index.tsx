...
const Index: NextPage = () => {
    ...
    const inProgressBills = bills?.filter((bill: any) => bill.Status === 'Repair Completed');
	const {
		data: technicians,
	...
	const downloadTableAsCSV = (table: any) => {
		...
		link.download = 'Repair Completed.csv';
		link.click();
	};
	const downloadTableAsPDF = async (table: HTMLElement) => {
		try {
			...
			const title = 'Repair Completed Report';
			pdf.setFontSize(16);
			pdf.setFont('helvetica', 'bold');
			...
			pdf.save('Repair Completed Report.pdf');
		} catch (error) {
	...
	const downloadTableAsPNG = async () => {
		try {
			...
			const link = document.createElement('a');
			link.href = dataUrl;
			link.download = 'Repair Completed.png';
			link.click();
		} catch (error) {
	...
	const downloadTableAsSVG = async () => {
		try {
			...
			const link = document.createElement('a');
			link.href = dataUrl;
			link.download = 'Repair Completed.svg';
			link.click();
		} catch (error) {
