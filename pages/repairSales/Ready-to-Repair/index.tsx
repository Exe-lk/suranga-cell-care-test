...
const Index: NextPage = () => {
    ...
    const inProgressBills = bills?.filter((bill: any) => bill.Status === 'Ready to Repair');
	const {
		data: technicians,
	...
	const downloadTableAsCSV = (table: any) => {
		...
		link.download = 'Ready to Repair.csv';
		link.click();
	};
	const downloadTableAsPDF = async (table: HTMLElement) => {
		try {
			...
			const title = 'Ready to Repair Report';
			pdf.setFontSize(16);
			pdf.setFont('helvetica', 'bold');
			...
			pdf.save('Ready to Repair Report.pdf');
		} catch (error) {
	...
	const downloadTableAsPNG = async () => {
		try {
			...
			const link = document.createElement('a');
			link.href = dataUrl;
			link.download = 'Ready to Repair.png';
			link.click();
		} catch (error) {
	...
	const downloadTableAsSVG = async () => {
		try {
			...
			const link = document.createElement('a');
			link.href = dataUrl;
			link.download = 'Ready to Repair.svg';
			link.click();
		} catch (error) {
