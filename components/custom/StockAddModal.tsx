...
const StockAddModal: FC<StockAddModalProps> = ({ id, isOpen, setIsOpen, quantity }) => {
	...
	const { data: stockInData, isSuccess, refetch: refetchCurrentItem } = useGetItemDisByIdQuery(id);
	...
	const [stockInOuts, setStockInOuts] = useState<any[]>([]);
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			try {
				...
				try {
					// Fetch current quantity from database instead of using stale prop
					const { data: currentItemData } = await refetchCurrentItem();
					const currentQuantity = currentItemData?.quantity || 0;
					const updatedQuantity = parseInt(currentQuantity) + parseInt(values.quantity);
					await addstockIn({
						...values,
						code: generatedCode,
						barcode: generatedbarcode,
